...
interface SupabaseCallback {
    ...
    suspend fun messageHandler(message: Message)
}
...
object SupabaseService {
    ...
    private suspend fun joinGame(
        ...
        val playerReadyJob = gameChannel.broadcastFlow<GameEvent>(BroadcastEvent.GAME_EVENT.name)
            .onEach { event ->
                println("SupabaseService joinGame handler $event")
                when(event.type) {
        ...
        val container = mutableListOf<Message>()
//callbackHandler?.recivedMessage(container)

        val sendMess = gameChannel.broadcastFlow<Message>(BroadcastEvent.SEND_MESSAGE.name)
            .onEach { mess ->
                println("MESSGAE: $mess")
                container.add(mess)
                callbackHandler?.messageHandler(message = mess)

            }.launchIn(coroutineScope)
        _gameJobs.add(sendMess)
    ...
    suspend fun sendMessage(message: Message) {
        ...
        sendMessageToGame(BroadcastEvent.SEND_MESSAGE, Json.encodeToJsonElement(message).jsonObject)
    }
